...
export function PortalSiteHeader({
  ...
  React.useEffect(() => {
    function handleOutsideClick(event: MouseEvent) {
      ...
      if (!target.closest("[data-portal-header-menu]")) {
        setMenuOpen(false)
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-2 sm:gap-4" data-portal-header-menu>
        <a
          href="/system-status"
        ...
        <div className="sm:hidden">
          <button
            ...
            className={cn(
              "inline-flex size-8 items-center justify-center rounded-md border border-[rgba(255,251,247,0.09)] bg-[#171616] text-[#e5e2e1] transition-all duration-300",
          ...
          <div
            className={cn(
              "pointer-events-none absolute right-3 top-[calc(100%+6px)] w-[220px] origin-top-right rounded-xl border border-[rgba(255,251,247,0.08)] bg-[#151414]/95 p-2 shadow-[0_16px_50px_rgba(0,0,0,0.5)] backdrop-blur-xl transition-all duration-200",
              menuOpen ? "pointer-events-auto translate-y-0 scale-100 opacity-100" : "translate-y-1 scale-[0.98] opacity-0"
            )}
            role="menu"
